...
import {
  ...
  Trophy, Crown, Eye, List, FolderX, ChevronRight, Info, Folder, UserCog, ArrowUpDown, Tag
} from 'lucide-react';
...
export default function App() {
  ...
  const handleTagSave = async (photo, newTags) => {
    ...
    if (selectedPhoto && selectedPhoto.id === photo.id) {
       setSelectedPhoto(prev => ({ ...prev, tags: sortedTags }));
    }
    setEditingPhoto(null);
    showToast(`태그 저장 완료! (+${POINTS.TAG_EDIT}점)`);

    try { 
        await updateDoc(doc(db, 'photos', photo.id), { tags: sortedTags }); 
        await updateDoc(doc(db, 'users', user.uid), { tagEditCount: increment(1) });
  ...
  const handleUpdateDesc = async (photoId, currentDesc) => {
    ...
    if (newDesc !== null && newDesc !== currentDesc) { 
      ...
      if (selectedPhoto && selectedPhoto.id === photoId) { setSelectedPhoto(prev => ({ ...prev, desc: newDesc })); }
      try { await updateDoc(doc(db, 'photos', photoId), { desc: newDesc }); showToast('수정 완료!'); } catch (e) { alert(e.message); } 
    }
  };
  ...
  const handleUpdateYear = async (photoId, currentYear) => {
    ...
    if (newYear !== null && newYear !== currentYear) {
      ...
      if (selectedPhoto && selectedPhoto.id === photoId) { setSelectedPhoto(prev => ({ ...prev, photoYear: newYear })); }
  ...
  if (loading) {
    return <div className="min-h-screen w-full bg-gray-200 flex justify-center items-center"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-600"></div></div>;
  }
...
// ★ [MyPageTab] 백지화 방지(loading check) + 대시보드 복구
function MyPageTab({ userData, photos, members, collections, renameCollection, onOpenAlbum, onPhotoClick }) {
  // 1. 데이터 로딩 중 방어 (백지화 해결)
  if (!userData) {
    return (
      <PageLayout><div className="flex h-full items-center justify-center flex-col gap-4"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div><p className="text-gray-400 text-sm">내 정보를 불러오는 중...</p></div></PageLayout>
    );
  }
  ...
  const totalUsers = members.length || 1; // 0 나누기 방지
