...
export default function App() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        if (docSnap.exists()) {
          ...
          // 🚨 비정상: 로그인 됐는데 DB가 비었음 (초기화 됨) -> 강제 로그아웃
  ...
  // ★ [2] 백지화 방지 (로그인은 됐는데 데이터가 없을때 -> 스피너)
