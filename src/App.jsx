...
// ★ 태그 정렬기 (숫자 우선 - 안전장치 포함)
const sortTagsSmart = (tags) => {
  if (!tags || !Array.isArray(tags)) return []; // 빈 배열 방어
...
const calculateUserScore = (userDoc, stats) => {
  if (!userDoc || !userDoc.id) return 0; // 유저 정보 없으면 0점
...
export default function App() {
  ...
  const [appLoading, setAppLoading] = useState(false); // 작업 중 로딩
  ...
  // ★ [2차 방어선] 로그인은 됐는데 userData가 아직 안 왔을 때 (백지화 방지 핵심)
  if (user && !userData) {
    return <div className="min-h-screen w-full bg-gray-200 flex justify-center items-center"><LoadingSpinner msg="회원 정보를 불러오는 중..." /></div>;
  }
...
// ... [서브 컴포넌트들은 v22와 동일하므로 생략하지 않고 전체 포함]
